# Skillura — coaching esport

Site vitrine statique (HTML / CSS / JS, sans build) hébergé sur GitHub Pages,
domaine `skillura.shop`.

> Généré depuis `C:\Users\souha\coaching-sites-factory` (fichier `sites/skillura.mjs`).
> Pour une modification de contenu, éditez ce fichier puis relancez `node build.mjs skillura` :
...
| 🟠 Important | Adresse `contact@skillura.shop` : créer une redirection e-mail chez Namecheap (*Domain List → Manage → Redirect Email*). | Namecheap |
...
| 🟡 Plus tard | Tarifs (25 / 69 / 139 €) et contenu des formules à ajuster à votre offre réelle. | `index.html` `#tarifs` |
| 🟡 Plus tard | Témoignages : n’en ajoutez que des vrais, avec l’accord des personnes. | — |

## Description de l’activité (Stripe, annuaires…)

```
Coaching en jeux vidéo compétitifs (esport), 100 % en ligne : séances individuelles en visioconférence et sessions en petit groupe portant sur les mécaniques de jeu, la prise de décision et la lecture de partie, l’analyse de replays enregistrés par le joueur, et la régularité d’entraînement. Les joueurs suivent un programme de 8 à 12 semaines avec des analyses écrites de leurs parties. Aucun compte de jeu, objet virtuel ou service de progression de compte n’est vendu, et aucun classement n’est garanti. Les prestations sont vendues sous forme d’abonnements mensuels sans engagement, de 25 € à 139 € par mois, résiliables à tout moment. Aucun produit physique n’est vendu ni expédié. Site : skillura.shop
```
